...
PDEPEND="
	...
	previewer? ( >=gnome-extra/sushi-3.38.0 )
	sendto? ( >=gnome-extra/nautilus-sendto-3.0.1 )
...
src_prepare() {
	...
	if ! use vanilla-menu; then
		if ! use vanilla-menu-compress; then
			eapply "${FILESDIR}"/${PN}-3.38.0-use-old-compress-extension.patch
			eapply "${FILESDIR}"/${PN}-3.38.0-reorder-context-menu-rebased.patch
		else
			eapply "${FILESDIR}"/${PN}-3.38.0-reorder-context-menu.patch
		fi
